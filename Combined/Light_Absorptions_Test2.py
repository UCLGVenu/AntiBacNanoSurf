...
def get_kx_layer(n_layer, k0, angle):
...
def func2(radii, height_in):
    ...
    for p in range(len(bot_rs)):
        ...
        angle = (np.pi / 180) * 0
        hmax = height_in[p]
        heights = np.arange(100e-9, hmax+(1e-9), 1e-9)
        for lp in range(2):
            for q in range(len(heights)):
                ...
                kx_layer1 = get_kx_layer(n_bot, k0, angle)
                kx_layer2 = get_kx_layer(n_si, k0, angle)
                ...
                for i in range(sections - 1):
                    ...
                    kx_layer1 = get_kx_layer(n_layer, k0, angle)
                ...
                kx_layer = get_kx_layer(n_top, k0, angle)
        ...
        results[round(bot_r+coat, 12)] = [heights, A_normalised]
        ...
        plt.title('R,T,A varying height - angle 20, pitch 200, coat 10, r 30-20')
        plt.show()'''
