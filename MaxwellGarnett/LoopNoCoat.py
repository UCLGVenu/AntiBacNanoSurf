...
for lp in range(2):
    for q in range(len(var)):
        ...
        bot_r = 20e-9 # Radius of INNER PILLAR (Substrate only)
        top_r = bot_r * top_fraction[q]
        pitch = 200e-9
        height = 500e-9
        kz = k0 * np.sin(angle)
        ...
        if lp == 0:
            ...
            Rs_nocoat.append(R_avg)
            Ts_nocoat.append(T_avg)
            As_nocoat.append(A_avg)
...
plt.title('R,T,A varying Top Radius - A 0, Base R 30, C 10, H 500, P 200')
plt.xlabel('Top Radius / Bot Radius')
plt.ylabel('Intensity')
plt.show()

A_normalised = [(As[i] - As_nocoat[i]) * 100 for i in range(len(As))]
An_avg = sum(A_normalised) / len(A_normalised)
plt.plot(var, A_normalised)
# plt.axhline(0, color = 'black')
# plt.axhline(An_avg, color = 'red')
plt.title('Normalised Absorption varying Top Radius - A 0, Base R 40, C 10, H 500, P 200')
plt.ylabel('Absorbed Intensity in TiO2 / %')
plt.xlabel('Top Radius / Base Radius')
plt.show()
...
plt.title('\"Useful\" light for TiO2 Diffusion Length 10nm')
plt.ylabel('% of incident light leading to (possible) ROS production')
plt.xlabel('Coating Thickness / m')
plt.show()
